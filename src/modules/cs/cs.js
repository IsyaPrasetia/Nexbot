...
// ============================================================
// Sinyal broadcast untuk health-monitor (bukan dipakai guard inner!)
// health-monitor membaca file ini untuk tahu CS sedang bulk/broadcast,
// lalu MENUNDA restart app supaya tidak memotong kiriman yang berjalan.
// File TERPISAH dari blast-flag.json (guard BLASTER) agar tidak mengganggu
// logika handover auto-reply di cs.js.
// ============================================================
const BLAST_MONITOR_FILE = CFG.files.monitorFile;
function touchBlastMonitor(active) {
    try {
        fs.writeFileSync(BLAST_MONITOR_FILE, JSON.stringify({ active: !!active, ts: Date.now() }));
    } catch (e) { /* file/aplikasi tidak bisa menulis — abaikan */ }
}
...
async function jalankanBroadcast(replySock, userJid, broadcastData, namaAdmin, targetSock, daftarGrupTarget) {
    ...
    for (const grup of daftarGrupTarget) {
        touchBlastMonitor(true); // jaga sinyal tetap fresh (broadcast bisa > 2 menit)
        if (broadcastStop) {
...
const TIMEOUT_BOT = 30 * 60 * 1000;              // 30 menit auto-reset ke BOT_MODE
const DELAY_MIN = 5000;                          // Minimum jeda antar grup (5 detik)
const DELAY_MAX = 20000;                         // Maksimum jeda antar grup (20 detik)
const TIMEOUT_MENU_SAPAAN = 15 * 60 * 1000;      // 15 menit cooldown sapaan setelah pilih menu
...
async function jalankanBotAdmin(namaAdmin, isBulkOnly = false) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            delete connectedAt[namaAdmin];
            const statusCode = lastDisconnect?.error?.output?.statusCode;
            const shouldReconnect = statusCode !== DisconnectReason.loggedOut;
            if (statusCode === DisconnectReason.loggedOut) {
                const connFile = path.join(sesDir, 'connected_at.json');
            ...
            const connFile = path.join(sesDir, 'connected_at.json');
            if (connectedAt[namaAdmin] == null) {
                let persisted = null;
                let birthtime = null;
                try { persisted = JSON.parse(fs.readFileSync(connFile, 'utf8')).ts || null; } catch {}
                try { birthtime = fs.statSync(sesDir).birthtime.getTime() || null; } catch {}
                connectedAt[namaAdmin] = persisted || birthtime || Date.now();
                if (!persisted) {
                    try { fs.writeFileSync(connFile, JSON.stringify({ ts: connectedAt[namaAdmin] }, null, 2), 'utf8'); } catch {}
                }
            }
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        try {
            ...
            if (userJid === GRUP_PROMO) {
                ...
                // Perintah broadcast diketik dari nomor admin itu sendiri (fromMe=true).
                // Jangan skip pesan sendiri JIKA itu perintah (diawali '!') supaya admin2
                // bisa memicu broadcast lewat nomornya sendiri.
                const isPerintah = /^!/.test(incomingText.trim());
                if (isFromMe && !isPerintah) return; // skip balasan/pesan sendiri non-perintah
                // Hanya socket yang benar-benar terhubung yang boleh memproses promosi.
                // Cegah socket OFFLINE (mis. admin1 yang sedang reconnect) mengunci
                // broadcast & memblokir socket online lain via pendingBroadcast global.
                if (!connectedAdmins[namaAdmin]) return;
            ...
            if (session.mode === 'HUMAN_MODE') {
                ...
                if (selisihWaktu > TIMEOUT_BOT) {
                    ...
                    // JANGAN update last_interaction dari pesan peserta saat HUMAN_MODE.
                    // last_interaction di sini mencerminkan momen terakhir admin membalas manual.
                    // Dengan begitu, begitu admin berhenti membalas > TIMEOUT_BOT, bot otomatis
                    // kembali BOT_MODE meskipun peserta tetap mengetik (fix: menu seperti '1'
                    // tak terbalas selamanya karena timer tak pernah timeout).
                    return; 
                }
            }
            ...
            if (command === '1') {
                ...
                await sendReply(sock, userJid, { 
                    text: menuTeks('menu_1a', "Halo kak👋🏻.\nKegiatan saat ini yang dapat diikuti :\n\n⚠️ *Arteria Medpro*\n\n*Kegiatan 30-31 Agustus*\n1. MANAJEMEN PERAWATAN LUKA KRONIK KANKER PAYUDARA 📅 30 Agustus 2026\n🔥 *2 SKP ALL NAKES KEMENKES RI*\n\n2. MANAJEMEN PERAWATAN LUKA KRONIK DIABETIC FOOT ULCER 📅 31 Agustus 2026\n🔥 *2 SKP ALL NAKES KEMENKES RI*\n\n🚩 *Pondok Sehat Indonesia (International Webinar)*\n\n*Kegiatan 5-6 September*\n3. MANAGEMENT OF SEPTIC AND CARDIOGENIC SHOCK 📅 5 September 2026\n🔥 *3 SKP KEMENKES RI*\n\n4. MANAGEMENT OF HEMORRHAGIC SHOCK 📅 6 September 2026\n🔥 *3 SKP KEMENKES RI*\n\nAtau kunjungi https://pondoksehatindonesia.org/\n\n━━━━━━━━━━━━━━━\n\n🚨 *KUOTA WEBINAR TERBATAS. 2 WEBINAR ARTERIA MEDPRO (30-31 AGUSTUS 2026)*‼️\n\nWEBINAR NASIONAL \"MANAJEMEN PERAWATAN LUKA KRONIK KANKER PAYUDARA & DIABETIC FOOT ULCER\"\n\n🎁 IKUTI 2 WEBINAR LANGSUNG\n\n💰 Total Harga : *Rp.29.000,-* (Daftar 2 Webinar sekaligus)\n📚 Total: *4 SKP*\n🎯 Peserta: *ALL NEKES*\n\nDAFTAR 👉 https://forms.gle/HRLcaBXvExZFuDMXA\n\nInfo Pendaftaran:\nwww.arteriamedpro.com\n\n━━━━━━━━━━━━━━━\n\n🚨 *KUOTA WEBINAR TERBATAS. 2 WEBINAR PONDOK SEHAT INDONESIA (5-6 SEPTEMBER 2026)*‼️\n\nWEBINAR INTERNATIONAL \"MANAGEMENT OF SEPTIC AND CARDIOGENIC SHOCK & MANAGEMENT OF HEMORRHAGIC SHOCK\"\n\n🎁 IKUTI 2 WEBINAR LANGSUNG\n\n💰 Biaya Webinar : *Rp.45.000,-*\n📚 Total: *6 SKP*\n🎯 Peserta: *ALL NEKES*\n\nDAFTAR 👉 https://forms.gle/gGJ3ncr1ikBJF52W8\n\nInfo Pendaftaran:\nhttps://pondoksehatindonesia.org/promosi")
                });
                await sendReply(sock, userJid, { 
                    text: menuTeks('menu_1b', "📝 *TERTARIK UNTUK MENDAFTAR?*\n\n• Ketik *2* untuk panduan pendaftaran mandiri (via website)\n• Ketik *3* untuk pendaftaran langsung (isi form di chat ini)\n\n_Bisa ketik *0* untuk kembali ke Menu Utama._")
                });
            } 
            else if (command === '2') {
                ...
                await sendReply(sock, userJid, { 
                    text: menuTeks('menu_2a', "📋 *CARA DAFTAR WEBINAR:*\n\n*1. Lakukan Pembayaran Sesuai Kategori:*\n  • *Webinar Internasional (PT Pondok Sehat Indonesia):*\n  Transfer ke *No. Rek [Lihat menu_texts.json]* a.n *PT. Pondok Sehat Indonesia*\n  • *Webinar Nasional (Arteria Medpro):*\n  Transfer ke *No. Rek [Lihat menu_texts.json]* a.n *CV. Arteria Medika Profesional*\n\n*2. Akses Tautan Registrasi Sesuai Kategori:*\n  • *Webinar Internasional (PSI):*\n  Buka tautan website: https://www.pondoksehatindonesia.com\n  • *Webinar Nasional (Arteria):*\n  Buka tautan website: https://www.arteriamedpro.com\n\n*3.* Pilih webinar yang akan diikuti pada halaman utama.\n*4.* Klik menu info selanjutnya, lalu klik tombol *\"Klik untuk daftar\"*.\n*5.* Isi lengkap seluruh format registrasi yang disediakan.\n*6.* Pastikan Anda bergabung dengan grup/saluran resmi melalui tautan yang tertera di akhir pengisian registrasi.\n\n_Bisa ketik *0* untuk kembali ke Menu Utama._")
                });
            } 
            else if (command === '3') {
                ...
                await sendReply(sock, userJid, { 
                    text: menuTeks('menu_3a', "✍️ *PENDAFTARAN LANGSUNG*\n\nBaik kak, untuk melakukan pendaftaran langsung, silakan lengkapi data pada form yang akan kami kirimkan di pesan berikutnya.\n\nMohon sertakan juga *screenshot bukti pembayaran* yang telah dilakukan.\n\n📌 *Informasi Pembayaran:*\n• *Webinar Internasional (PT Pondok Sehat Indonesia):*\n  Transfer ke *No. Rek [Lihat menu_texts.json]* a.n *PT. Pondok Sehat Indonesia*\n\n━━━━━━━━━━━━━━━\n\n• *Webinar Nasional (Arteria Medpro):*\n  Transfer ke *No. Rek [Lihat menu_texts.json]* a.n *CV. Arteria Medika Profesional*\n\n_Bisa ketik *0* untuk kembali ke Menu Utama._")
                });
                await sendReply(sock, userJid, {
                    text: menuTeks('menu_3b', "📝 *FORM PENDAFTARAN LANGSUNG*\n\nSilakan *salin* format di bawah ini, isi dengan data Anda, lalu kirim kembali ke chat ini beserta bukti transfer:\n\n• *Nama Lengkap* :\n• *Email Plataran Sehat* :\n• *NIK KTP* :\n• *Profesi* :\n• *Webinar Tanggal* :\n\n_Mohon pastikan seluruh data terisi dengan benar agar pendaftaran dapat segera kami proses._")
                });
            }
            else if (command === '4') {
                ...
                await sendReply(sock, userJid, { 
                    text: menuTeks('menu_4a', "⚠️ *KENDALA VERIFIKASI / ERROR PADA SISTEM*\n\nBaik kak, mohon bantuannya untuk melengkapi data pada form yang akan kami kirimkan di pesan berikutnya, guna mempercepat proses pengecekan oleh tim sistem kami.\n\n📌 *Catatan Tambahan:* Kami menjamin keamanan dan kerahasiaan seluruh data pribadi yang Anda kirimkan.\n\n💡 *Tips:* Jika Kakak mengalami kendala pada proses upload bukti transfer, mohon sertakan foto bukti transfer secara langsung di chat ini beserta data yang telah diisi, agar tim kami dapat segera membantu mengecek statusnya.\n\n_Bisa ketik *0* untuk kembali ke Menu Utama._")
                });
                await sendReply(sock, userJid, {
                    text: menuTeks('menu_4b', "📝 *FORM DATA VERIFIKASI*\n\nSilakan *salin* format di bawah ini, isi dengan data Anda, lalu kirim kembali ke chat ini:\n\n• *Nama Lengkap* :\n• *Email Plataran Sehat* :\n• *NIK KTP* :\n• *Profesi* :\n\n_Mohon pastikan seluruh data terisi dengan benar agar tim kami dapat segera membantu._")
                });
            } 
            else if (command === '5') {
                ...
                await sendReply(sock, userJid, { 
                    text: menuTeks('menu_5a', "🎟️ *FORM KLAIM VOUCHER WEBINAR*\n\nBaik kak, untuk memproses klaim voucher Anda, mohon lengkapi data pada form yang akan kami kirimkan di pesan berikutnya.\n\n📌 *Ketentuan Klaim:* Tim kami akan melakukan verifikasi kecocokan data Anda dengan sistem riwayat transaksi. Proses pengecekan antrean voucher membutuhkan waktu maksimal 1x24 jam.\n\n_Bisa ketik *0* untuk kembali ke Menu Utama._")
                });
                await sendReply(sock, userJid, {
                    text: menuTeks('menu_5b', "📝 *FORM DATA KLAIM VOUCHER*\n\nSilakan *salin* format di bawah ini, isi dengan data Anda, lalu kirim kembali ke chat ini:\n\n• *Nama Lengkap* :\n• *Email Plataran Sehat* :\n• *NIK KTP* :\n• *Profesi* :\n• *Judul Webinar & Tanggal* :\n\n_Mohon pastikan seluruh data terisi dengan benar agar proses klaim voucher dapat segera kami proses._")
                });
            }
            else if (command === '6') {
                ...
                await sendReply(sock, userJid, { 
                    text: menuTeks('menu_6a', "📋 *KETENTUAN TAMBAHAN*\n\n1. Verifikasi akan dilakukan H-2 sebelum hari pelaksanaan. Jika Kakak mendaftar kurang dari H-2, verifikasi akan diproses secepatnya.\n\n2. Seluruh informasi mengenai webinar akan diberikan pada grup webinar terkait. Pastikan Kakak sudah bergabung di grupnya ya.\n\n3. Untuk kendala verifikasi atau upload bukti transfer, silakan gunakan opsi *4* (Verifikasi / Error).\n\n4. Mohon kesabaran Bapak/Ibu untuk mendapat respon dari admin. Kami memprioritaskan penanganan dari yang paling lama terlebih dahulu.\n\n5. Jika pada hari H kegiatan terdapat kendala, mohon untuk segera menghubungi admin melalui opsi *7* (Customer Care).\n\n_Bisa ketik *0* untuk kembali ke Menu Utama._")
                });
            }
            else if (command === '7') {
                ...
                await sendReply(sock, userJid, { 
                    text: menuTeks('menu_7a', "🧑‍💼 *LAYANAN RESPONS LANGSUNG (CUSTOMER CARE)*\n\nPertanyaan Anda telah kami teruskan ke tim Customer Care Layanan Terpadu. Mohon tunggu beberapa saat, petugas kami akan segera menyapa dan memberikan solusi terbaik untuk Anda secara langsung di room chat ini.")
                });
            }
...
// ================================================================
// 🌉 BRIDGE DASHBOARD (http://127.0.0.1:5591) - dibaca oleh panel kontrol
// ================================================================
...
const MENU_BAWAAN = {
    'menu_1a': "Halo kak👋🏻.\nKegiatan saat ini yang dapat diikuti :\n\n⚠️ *Arteria Medpro*\n\n*Kegiatan 30-31 Agustus*\n1. MANAJEMEN PERAWATAN LUKA KRONIK KANKER PAYUDARA 📅 30 Agustus 2026\n🔥 *2 SKP ALL NAKES KEMENKES RI*\n\n2. MANAJEMEN PERAWATAN LUKA KRONIK DIABETIC FOOT ULCER 📅 31 Agustus 2026\n🔥 *2 SKP ALL NAKES KEMENKES RI*\n\n🚩 *Pondok Sehat Indonesia (International Webinar)*\n\n*Kegiatan 5-6 September*\n3. MANAGEMENT OF SEPTIC AND CARDIOGENIC SHOCK 📅 5 September 2026\n🔥 *3 SKP KEMENKES RI*\n\n4. MANAGEMENT OF HEMORRHAGIC SHOCK 📅 6 September 2026\n🔥 *3 SKP KEMENKES RI*\n\nAtau kunjungi https://pondoksehatindonesia.org/\n\n🚨 *KUOTA WEBINAR TERBATAS. 2 WEBINAR ARTERIA MEDPRO (30-31 AGUSTUS 2026)*‼️\n\nWEBINAR NASIONAL \"MANAJEMEN PERAWATAN LUKA KRONIK KANKER PAYUDARA & DIABETIC FOOT ULCER\"\n\n🎁 IKUTI 2 WEBINAR LANGSUNG\n\n💰 Total Harga : *Rp.29.000,-* (Daftar 2 Webinar sekaligus)\n📚 Total: *4 SKP*\n🎯 Peserta: *ALL NEKES*\n\nDAFTAR 👉 https://forms.gle/HRLcaBXvExZFuDMXA\n\nInfo Pendaftaran:\nwww.arteriamedpro.com\n\n━━━━━━━━━━━━━━━\n\n🚨 *KUOTA WEBINAR TERBATAS. 2 WEBINAR PONDOK SEHAT INDONESIA (5-6 SEPTEMBER 2026)*‼️\n\nWEBINAR INTERNATIONAL \"MANAGEMENT OF SEPTIC AND CARDIOGENIC SHOCK & MANAGEMENT OF HEMORRHAGIC SHOCK\"\n\n🎁 IKUTI 2 WEBINAR LANGSUNG\n\n💰 Biaya Webinar : *Rp.45.000,-*\n📚 Total: *6 SKP*\n🎯 Peserta: *ALL NEKES*\n\nDAFTAR 👉 https://forms.gle/gGJ3ncr1ikBJF52W8\n\nInfo Pendaftaran:\nhttps://pondoksehatindonesia.org/promosi",
    'menu_1b': "📝 *TERTARIK UNTUK MENDAFTAR?*\n\n• Ketik *2* untuk panduan pendaftaran mandiri (via website)\n• Ketik *3* untuk pendaftaran langsung (isi form di chat ini)\n\n_Bisa ketik *0* untuk kembali ke Menu Utama._",
    'menu_2a': "📋 *CARA DAFTAR WEBINAR:*\n\n*1. Lakukan Pembayaran Sesuai Kategori:*\n  • *Webinar Internasional (PT Pondok Sehat Indonesia):*\n  Transfer ke *No. Rek [Lihat menu_texts.json]* a.n *PT. Pondok Sehat Indonesia*\n  • *Webinar Nasional (Arteria Medpro):*\n  Transfer ke *No. Rek [Lihat menu_texts.json]* a.n *CV. Arteria Medika Profesional*\n\n*2. Akses Tautan Registrasi Sesuai Kategori:*\n  • *Webinar Internasional (PSI):*\n  Buka tautan website: https://www.pondoksehatindonesia.com\n  • *Webinar Nasional (Arteria):*\n  Buka tautan website: https://www.arteriamedpro.com\n\n*3.* Pilih webinar yang akan diikuti pada halaman utama.\n*4.* Klik menu info selanjutnya, lalu klik tombol *\"Klik untuk daftar\"*.\n*5.* Isi lengkap seluruh format registrasi yang disediakan.\n*6.* Pastikan Anda bergabung dengan grup/saluran resmi melalui tautan yang tertera di akhir pengisian registrasi.\n\n_Bisa ketik *0* untuk kembali ke Menu Utama._",
    'menu_3a': "✍️ *PENDAFTARAN LANGSUNG*\n\nBaik kak, untuk melakukan pendaftaran langsung, silakan lengkapi data pada form yang akan kami kirimkan di pesan berikutnya.\n\nMohon sertakan juga *screenshot bukti pembayaran* yang telah dilakukan.\n\n📌 *Informasi Pembayaran:*\n• *Webinar Internasional (PT Pondok Sehat Indonesia):*\n  Transfer ke *No. Rek [Lihat menu_texts.json]* a.n *PT. Pondok Sehat Indonesia*\n\n━━━━━━━━━━━━━━━\n\n• *Webinar Nasional (Arteria Medpro):*\n  Transfer ke *No. Rek [Lihat menu_texts.json]* a.n *CV. Arteria Medika Profesional*\n\n_Bisa ketik *0* untuk kembali ke Menu Utama._",
    'menu_3b': "📝 *FORM PENDAFTARAN LANGSUNG*\n\nSilakan *salin* format di bawah ini, isi dengan data Anda, lalu kirim kembali ke chat ini beserta bukti transfer:\n\n• *Nama Lengkap* :\n• *Email Plataran Sehat* :\n• *NIK KTP* :\n• *Profesi* :\n• *Webinar Tanggal* :\n\n_Mohon pastikan seluruh data terisi dengan benar agar pendaftaran dapat segera kami proses._",
    'menu_4a': "⚠️ *KENDALA VERIFIKASI / ERROR PADA SISTEM*\n\nBaik kak, mohon bantuannya untuk melengkapi data pada form yang akan kami kirimkan di pesan berikutnya, guna mempercepat proses pengecekan oleh tim sistem kami.\n\n📌 *Catatan Tambahan:* Kami menjamin keamanan dan kerahasiaan seluruh data pribadi yang Anda kirimkan.\n\n💡 *Tips:* Jika Kakak mengalami kendala pada proses upload bukti transfer, mohon sertakan foto bukti transfer secara langsung di chat ini beserta data yang telah diisi, agar tim kami dapat segera membantu mengecek statusnya.\n\n_Bisa ketik *0* untuk kembali ke Menu Utama._",
    'menu_4b': "📝 *FORM DATA VERIFIKASI*\n\nSilakan *salin* format di bawah ini, isi dengan data Anda, lalu kirim kembali ke chat ini:\n\n• *Nama Lengkap* :\n• *Email Plataran Sehat* :\n• *NIK KTP* :\n• *Profesi* :\n\n_Mohon pastikan seluruh data terisi dengan benar agar tim kami dapat segera membantu._",
    'menu_5a': "🎟️ *FORM KLAIM VOUCHER WEBINAR*\n\nBaik kak, untuk memproses klaim voucher Anda, mohon lengkapi data pada form yang akan kami kirimkan di pesan berikutnya.\n\n📌 *Ketentuan Klaim:* Tim kami akan melakukan verifikasi kecocokan data Anda dengan sistem riwayat transaksi. Proses pengecekan antrean voucher membutuhkan waktu maksimal 1x24 jam.\n\n_Bisa ketik *0* untuk kembali ke Menu Utama._",
    'menu_5b': "📝 *FORM DATA KLAIM VOUCHER*\n\nSilakan *salin* format di bawah ini, isi dengan data Anda, lalu kirim kembali ke chat ini:\n\n• *Nama Lengkap* :\n• *Email Plataran Sehat* :\n• *NIK KTP* :\n• *Profesi* :\n• *Judul Webinar & Tanggal* :\n\n_Mohon pastikan seluruh data terisi dengan benar agar proses klaim voucher dapat segera kami proses._",
    'menu_6a': "📋 *KETENTUAN TAMBAHAN*\n\n1. Verifikasi akan dilakukan H-2 sebelum hari pelaksanaan. Jika Kakak mendaftar kurang dari H-2, verifikasi akan diproses secepatnya.\n\n2. Seluruh informasi mengenai webinar akan diberikan pada grup webinar terkait. Pastikan Kakak sudah bergabung di grupnya ya.\n\n3. Untuk kendala verifikasi atau upload bukti transfer, silakan gunakan opsi *4* (Verifikasi / Error).\n\n4. Mohon kesabaran Bapak/Ibu untuk mendapat respon dari admin. Kami memprioritaskan penanganan dari yang paling lama terlebih dahulu.\n\n5. Jika pada hari H kegiatan terdapat kendala, mohon untuk segera menghubungi admin melalui opsi *7* (Customer Care).\n\n_Bisa ketik *0* untuk kembali ke Menu Utama._",
    'menu_7a': "🧑‍💼 *LAYANAN RESPONS LANGSUNG (CUSTOMER CARE)*\n\nPertanyaan Anda telah kami teruskan ke tim Customer Care Layanan Terpadu. Mohon tunggu beberapa saat, petugas kami akan segera menyapa dan memberikan solusi terbaik untuk Anda secara langsung di room chat ini.",
    'menu_utama': "📋 Selamat datang di *Layanan Terpadu Terotomatisasi 📋*.\n\nAda yang bisa kami bantu hari ini? Silakan balas dengan mengetik *angka pilihan* di bawah ini:\n\n📌 *1* Info Promo Webinar\n📝 *2* Cara Pendaftaran\n✍️ *3* Pendaftaran Langsung\n⚠️ *4* Verifikasi / Error Pada Upload\n🎟️ *5* Claim Voucher\n📋 *6* Ketentuan Tambahan\n🧑‍💼 *7* Hubungi Customer Care\n\n💡 _Silakan balas langsung berupa angka 1, 2, 3, 4, 5, 6, atau 7._"
};
...
function qrInfoBridge(slot) {
    try {
        const f = config.stores.qr(slot, 'cs');
        ...
        try {
            if (req.method === 'GET' && req.url === '/status') {
                const slots = bridgeSlotList().map((s) => {
                    ...
                    const connected = !!(connectedAdmins[s] && adminSocks[s]);
                    const sock = adminSocks[s];
                    const u = sock && sock.user && sock.user.id ? String(sock.user.id).split('@')[0].split(':')[0] : null;
                    return {
                        slot: s,
                        connected,
                        connected_sec: connected && connectedAt[s] ? Math.max(0, (Date.now() - connectedAt[s]) / 1000) : 0,
                        nomor: u,
                        qr_fresh: q.fresh,
            ...
            if (req.method === 'POST' && req.url === '/reset') {
                ...
                try {
                    ...
                    // AUTO-SPAWN ulang socket agar QR baru langsung tampil (tanpa perlu /spawn manual)
                    setTimeout(() => { jalankanBotAdmin(name, name === 'admin2'); }, 3000);
            ...
            if (req.method === 'POST' && req.url === '/reset-all') {
                ...
                // AUTO-SPAWN ulang semua socket biar QR baru langsung tampil (tanpa perlu /spawn manual)
                for (const s of ['admin1', 'admin2', 'admin3']) {
                    setTimeout(() => { jalankanBotAdmin(s, s === 'admin2'); }, 3000);
                }
